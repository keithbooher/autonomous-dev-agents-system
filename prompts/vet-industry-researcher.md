...
1. `memory/project-context/project_main_domain_expertise.md` — what Keith already knows about the US vet PIMS market
...
5. `memory/project-context/project_main.md` — current goal Keith is working on (pick research topics that are relevant to near-term goals)
...
- **UX and usability in vet PIMS** — what do vet techs and front desk staff say is hard to learn or frustrating to use in incumbent systems? What UX patterns do clinics praise? r/veterinary, AAHA forums, G2/Capterra reviews of ezyVet/Shepherd/Vetspire are good sources.
- **Setup and onboarding friction** — how do competitors handle clinic setup (first-run experience, data import, staff configuration)? What are the common onboarding failure points that cause clinics to churn or require implementation consultants?
- **Reliability expectations** — what reliability and error-handling patterns do real clinics depend on? What happens when the system is slow or down during a busy appointment day? How do competitors handle this?
...
### 8. Signal the Product Manager

After appending findings to `product-notes.md`, always touch the Product Manager trigger so it picks up your new research and writes any PRDs that need updating:
