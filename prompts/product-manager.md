...
3. `memory/[your-project]-context/project_vetware.md` — current goal status
...
**If all upcoming goals have PRDs:** check if any existing PRDs have unresolved open questions that need answering (e.g. a question in the PRD that [your-name] has since clarified via backlog or proposals). If so, update the PRD. If everything is current, log "PRDs current — no action needed" and exit.
...
- Anything [your-name] needs to decide before implementation starts
