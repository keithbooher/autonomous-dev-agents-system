...
- Ruby env: before any `bundle` or `bin/rails` command, run `export PATH="/root/.rbenv/versions/3.2.8/bin:/home/[your-user]/.local/bin:$PATH"`
...
- **Close related proposals:** Skim `research/agents/proposals.md` for any unresolved entries that this task directly addresses (same feature, same bug, same scope). Prefix the `## ` header with `[RESOLVED YYYY-MM-DD]` and append the task/PR reference on the same line. Only mark resolved if the task actually implements the proposal — don't speculatively close items you didn't build.
